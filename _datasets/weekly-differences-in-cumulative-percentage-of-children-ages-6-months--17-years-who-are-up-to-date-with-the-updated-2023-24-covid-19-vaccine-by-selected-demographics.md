---
schema: data_rescue_project 
title: Weekly Differences in Cumulative Percentage of Children Ages 6 Months -17 Years who are up to date with the updated 2023-24 COVID-19 Vaccine, by Selected Demographics
organization: Centers for Disease Control and Prevention (CDC)
agency: Department of Health and Human Services
websites: data.cdc.gov
data_source: https://data.cdc.gov/Child-Vaccinations/Weekly-Differences-in-Cumulative-Percentage-of-Chi/qfiq-jir6/about_data
description: 
last_modified: 2026-01-25
dataset_source_status: 
metadata_available: No
metadata_url: 
category:
  - Health & Healthcare 
resources:
  - id: 2576
    url: https://www.datalumos.org/datalumos/project/243559/version/V1/view
    format: PDF, CSV
    status: Finished
    size: 0.0001
    download_date: 
    maintainer: DRP, DL
    notes: 
---
