---
title: World Factbook
organization: Central Intelligence Agency
agency: Central Intelligence Agency
websites: cia.gov
data_source: https://www.cia.gov/the-world-factbook/
description: Shutdown February 4, 2026 https://www.politico.com/news/2026/04/05/eulogy-for-the-cia-factbook-the-free-standard-for-world-facts-is-gone-00859250
last_modified: 2026-08-22
dataset_source_status: discontinued, tool removed
metadata_available: No
metadata_url: 
category:
  - Business & Economy 
  - Climate & Environment 
  - Communications 
  - Energy 
  - Health & Healthcare 
  - Housing & Community Development 
  - Infrastructure 
  - Military & Veterans Affairs 
  - National Security & Defense 
  - State & Policy 
  - Transportation 
resources:
  - id: 2999
    url: https://openfactbook.org/
    format: HTML, JSON
    status: Finished
    size: 
    download_date: 
    maintainer: OPEN, GITHB
    notes: OpenFactBook aggregates data from multiple authoritative sources to provide comprehensive and accurate information. Data sources CIA World Factbook, World Bank Open Data, REST Countries API. Raw datasets https://github.com/factbook/factbook.json. Updates are ongoing.
  - id: 3000
    url: https://web.archive.org/web/20260131095511/https://www.cia.gov/the-world-factbook/
    format: HTML, PDF
    status: Finished
    size: 
    download_date: 2026-01-31
    maintainer: WM
    notes: 
  - id: 4750
    url: https://databook.dataint.net/
    format: Webpage
    status: Finished
    size: 
    download_date: 
    maintainer: DATAINT
    notes: Databook is a browsable country reference, (not an exact archive of the Factbook's own files), covering ~250 countries. 41 of its 242 indicators carry the Factbook's own fields forward, kept alongside live sources (World Bank WDI, WHO GHO, UNESCO UIS, IPUParline, UNDP HDI, Aquastat, World Inequality Database) so those series don't simply stop in February 2026.
---
