...
try:
    resp = requests.get(
        url="https://findgovdata.org/files/last_seen.json",
        timeout=30,
    )
    ...
    df_expanded["download_urls"] = df_expanded["dcat.distribution"].apply(
        get_URLs_from_dist
    )

    df_expanded["last_seen"] = pd.to_datetime(
        df_expanded["last_seen"], format="ISO8601"
    )
    df_expanded["last_seen"] = df_expanded["last_seen"].dt.date

    df_expanded = df_expanded.drop(
        columns=["dcat.distribution", "distribution_count"]
    ).rename(columns={"dcat.landingPage": "source_url", "id": "hashed_id"})
    df_expanded.last_seen = df_expanded.last_seen.astype("str")

    ## Fetching the datasets already added to baserow
    rows_on_baserow = get_results_json(
    ...
    rows_to_toggle_status = rows_on_baserow[
        ~rows_on_baserow.hashed_id.isin(df_expanded.hashed_id)
    ]
    rows_to_toggle_status = rows_to_toggle_status[
        rows_to_toggle_status.status == "Missing from data.gov"
    ]
    ...
    if len(rows_to_toggle_status) > 0:
        for i in rows_to_toggle_status.id:
            requests.patch(
                f"https://baserow.datarescueproject.org/api/database/rows/table/1054/{i}/?user_field_names=true",
                headers={
                    "Authorization": f"Token {BASEROW_ACCESS_TOKEN}",
                    "Content-Type": "application/json",
                },
    ...
    status_returned_to_toggle = status_returned[
        ~status_returned.hashed_id.isin(df_expanded.hashed_id)
    ]
    ...
    if len(status_returned_to_toggle) > 0:
        ...
        print(
            f"{len(status_returned_to_toggle)} rows marked once again as missing in Baserow table"
        )
