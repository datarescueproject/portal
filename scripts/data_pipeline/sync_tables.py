...
VOLUNTEER_ACCESS_TOKEN = "blYJNfBWXUjkRVpv0byQKUQr2zoFIJQQ"
...
def get_results_json(url, api_key=BASEROW_ACCESS_TOKEN):
    ...
    if table.json()['next'] is not None:
        res.extend(get_results_json(table.json()['next'], api_key=api_key))
...
for t in tables_to_sync:
    rows_on_tracker = get_results_json(
        f"https://baserow.datarescueproject.org/api/database/rows/table/{t['source']}/?user_field_names=true",BASEROW_ACCESS_TOKEN)
    rows_on_volunteer = get_results_json(
        f"https://baserow.datarescueproject.org/api/database/rows/table/{t['destination']}/?user_field_names=true",VOLUNTEER_ACCESS_TOKEN)
